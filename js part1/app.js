...
// Циклы предусмотрены для многократного повторения участка кода. Например, необходимо перебрать все элементы списка и выполнить для каждого какое-то действие. В таком случае мы хотели бы выполнять это действие до тех пор, пока мы не дойдем до конца списка, следовательно, для цикла важно задать условие, при котором будет выполняться действие.Рассмотрим синтаксис цикла while:
// 	while (условие) {
//   		// код, тело цикла
// 	}
// 	Пока условие верно – будет выполняться код из тела цикла. В этом цикл похож с конструкцией if.
// Создадим цикл, который будет выводить числа от 1 до 10:
// 	let i = 1;
// 	while (i <= 10) {
//   		console.log( i );
//   	i++;
// 	}
// Каждое повторение тела цикла - итерация. В примере выше цикл совершит десять итераций. Если бы мы не изменяли значение i (i++), то цикл выполнялся бы бесконечно. Если для выполнения тела цикла необходимо, чтобы условие оставалось верным, то простейшим примером бесконечного цикла будет являться следующий:
// 	while (true) {
//   		// тело цикла
// 	}			// Дополнительную информацию о циклах вы можете найти по следующей ссылке: https://learn.javascript.ru/while-for
...
// i < 3 - условие (проверяется перед каждой итерацией цикла. Если условие false, цикл остановится);
// i++ - шаг (выполняется после тела цикла на каждой итерации перед проверкой условия);
// console.log(i); - тело цикла (выполняется до тех пор, пока условие true).

// Существует возможность пропустить некоторые части цикла for: https://learn.javascript.ru/while-for#propusk-chastey-for.
// Также цикл можно прервать в необходимый момент, применив конструкцию break: https://learn.javascript.ru/while-for#preryvanie-tsikla-break.
// Возможен пропуск итерации с помощью continue: https://learn.javascript.ru/while-for#continue.

// Существуют ситуации, когда необходимо выполнить код хотя бы один раз вне зависимости от выполнения условия. В таком случае вместо цикла while или for можно использовать do...while:
// 	do {
//  	 // тело цикла
// 	} while (условие);
// Цикл, описанный, таким образом, сначала выполняет тело, а затем проверяет условие.
// Для работы со свойствами объектов (для их перебора) можно использовать цикл for...in: https://learn.javascript.ru/object#the-for-in-loop.
// Другим вариантом перебора свойств объекта (в том числе элементов массива) является цикл for...of: https://developer.mozilla.org/ru/docs/Web/JavaScript/Reference/Statements/for...of#Обход_Array.

// Внутри тела цикла может содержаться неограниченное количество условных операторов, которые будут влиять на код и результат, получаемый после работы цикла.
// 	for (let i = 0; i <= 5; i++) {
//  	 if (i > 2) {
//    	 console.log(i);
//  	 }
// 	}
// В этом примере i выводится не в промежутке от 0 до 5, а начиная с 3, так как внутри цикла содержится условный оператор, ограничивающий эти значения.

// Пример 1. Рассмотрим программу, которая заполняет массив четными числами от 2 до 20.
// 	const arr = [];
// 	for (let i = 2; i <= 20; i++){
//  	 if (i % 2 === 0) arr.push(i); 
// 	}
...
// Пример 2. Рассмотрим программу, которая позволяет найти максимальный элемент массив и его индекс.
...
// Пример 3. Рассмотрим программу, находящую количество букв 'g' в строке.
...
// Зачастую нам надо повторять одно и то же действие во многих частях программы. Чтобы не повторять один и тот же код во многих местах, придуманы функции. Функции являются основными «строительными блоками» программы. Для создания функций мы можем использовать объявление функции.
// 	function имя (параметры) {
//  	 тело функции
// 	}
// Вначале идёт ключевое слово function, после него имя функции, затем cписок параметров в круглых скобках через запятую, и, наконец, код функции, также называемый «телом функции», внутри фигурных скобок. Функцию, объявленную таким образом, можно вызывать в любом месте программы. Пример. Предположим, в программе часто надо выполнять сложение двух чисел. Создадим функцию:
// 	function sum(a, b) {
// 	  return a + b;
// 	}
// В программе можно многократно вызвать эту функцию для нахождения суммы значений переменных:
// 	let x = 8;
// 	let y = 145;
...
// 	}
// Вызов функции:
// 	sayHello('Alice'); // вызов функции, будет выведено "Hello, Alice"
// 	sayHello('Maria'); // вызов функции, будет выведено "Hello, Maria"
// Если в функции отсутствует return, то функция возвращает значение undefined.
// Пример. Напишем функцию, которая умножает два числа.
// 	function mult(a, b) {
//  	 return a * b;
// 	}
// Если при вызове функции указать два аргумента, то функция возвратит верный результат,например,
// 	console.log(mult(2, 7)); // 14
// Если при вызове функции указать только один аргумент, или ни одного, то функция вернет NaN.
// 	console.log(mult(2)); // NaN
// 	console.log(mult()); // NaN
// Чтобы при вызове функции с одним аргументом или без аргументов возвращался результат 0, можно задать значения аргументов по умолчанию. Это делается так:
// 	function mult(a=0, b=0) {
// 	  return a * b;
// 	}
// Тогда если при вызове задавать значения аргументов, то значения по умолчанию будут игнорироваться. А если при вызове один из аргументов будет отсутствовать, ему будет дано значение по умолчанию равное 0, и тогда функция вернет значение 0.
// 	function mult(a=0, b=0) {
// 	  return a * b;
// 	}
...
// Имя каждой функции на странице должно быть уникальным. К именам функций также применимы основные правила, применяемые к именам переменным (camelCase, избегание зарезервированных слов и т.п.). Помимо этого рекомендуется использовать глаголы в названиях функций:
// 	get - "получает" что-то
// 	calc - "вычисляет"
// 	create - "создает"
// 	check - "проверяет" что-то (например, возвращает true/false) Имя функции должно понятно и чётко отражать, что она делает. Увидев её вызов в коде, вы должны тут же понимать, что она делает. Одна функция – одно действие. Функция должна делать только то, что явно подразумевается её названием. И это должно быть одним действием.
// 	Передаваемые значения копируются в параметры функции и становятся локальными переменными. Если параметр не указан, то его значением становится undefined. Если мы хотим задать параметру значение по умолчанию, мы должны указать его после =. Директива return может находиться в любом месте тела функции. Как только выполнение доходит до этого места, функция останавливается, и значение возвращается в вызвавший её код.

// Объявление функции, показанной ниже, называется Function Declaration
// function sum(a, b) {
//   return a + b;
// }
// Функцию, объявленную таким способом, можно вызвать в любом месте кода (даже до ее объявления).

// Бывают ситуации, когда результат функции необходимо присвоить в переменную. Тогда можно испольовать другой способ объявления - Function Expression.
// let sum = function (a, b) {
//   return a + b;
// }
// Основное отличие в этих способах объявления - Function Expression создаются в процессе выполнения выражения, в котором созданы.

// Существует ещё один способ создания функции, который, правда, используется очень редко. Он позволяет создавать функцию полностью «на лету» из строки, вот так:
...
// Одной из самых заметных новшеств современного JavaScript стало появление стрелочных функций (arrow function), которые иногда называют «толстыми» стрелочными функциями (fat arrow function). Стрелочные функции имеют удобный и компактный синтаксис. При объявлении таких функций используют особую комбинацию символов =>.
// let sum = (a, b) => a + b;